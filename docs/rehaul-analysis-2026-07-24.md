...
- **Onboarding:** 5-step gated wizard covering welcome, mic picker, model download offer, **forced hotkey test that echoes back the user's real first transcription ("It worked!")**, tray explainer. One of the strongest UX pieces in the app.
...
- **No version control:** gitignored inside another project's repo. Zero history, zero backup, one `rm -rf` from gone.
