...
- **Live Preview: watch the words appear while you are still speaking.** Between letting go of the key and seeing the text there was a silent second where nothing on screen proved Inkwell had heard you. Turn Live Preview on in General and the overlay grows to show what you are saying as you say it. The words are lowercase and unpunctuated because they come from a second, much faster model; what actually gets pasted is produced the same way it always was, by the model you chose, with its casing and punctuation intact. Nothing about your dictation changes, you can just see it happening. Off by default, and turning it on downloads a 73 MB English model. The last word or two will not appear before you release the key, which is a limit of how streaming recognition works and is exactly what the main model is there to fix.
- **The Fn (globe) key, right Command, right Option and right Control can be the hotkey.** Pick them with one click under the hotkey field. These keys never reach the normal hotkey system, because a modifier alone produces no key event, so Inkwell watches for them separately, and only on macOS. The watch is listen-only: the key keeps working as a modifier everywhere else. Two caveats come with the territory: macOS gives the globe key its own job by default (set "Press globe key to" to Do Nothing in System Settings, or dictating will also open the emoji picker), and the left-side modifiers are deliberately not offered, since a hotkey on left Command would fire on every copy and paste. Requires the Accessibility permission Inkwell already uses for pasting.
