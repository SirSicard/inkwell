//! Disfluency removal: the fillers and stutters speech carries and writing does not. Ported from
//! Inkwell 0.2's `cleanup.rs`, tests included; the code is unchanged.
//!
//! Runs before the style transform (stage 6), because removing a leading "Um" leaves the next
//! word lowercase and style is the stage that fixes casing. A mode can turn it off.
//!
//! Deliberately conservative. Every word here is one that carries no meaning in writing. "like",
//! "you know", "I mean", "actually", "basically" and "right" are **not** removed, even though other
//! dictation tools strip some of them: each has real uses ("a tool like this", "you know the
//! answer"), and a dictation tool that silently deletes meaningful words is worse than one that
//! leaves an "um" in. Removing a word here must never change what a sentence says.

/// Interjections with no written meaning. Matched whole-word, case-insensitive.
const FILLERS: &[&str] = &[
    "um", "umm", "ummm", "uh", "uhh", "uhhh", "erm", "hmm", "hmmm", "mmm", "mhm", "uhm", "eh",
];
...
const STUTTER_SAFE: &[&str] = &[
    "i", "the", "a", "an", "we", "you", "they", "to", "of", "and", "but", "so", "in", "on", "at",
    "is", "are", "was", "were", "my", "your", "this", "it",
];
...
mod tests {
    ...
    fn removes_the_filler_from_a_real_transcript() {
        // The case that motivated the module: an "Um" went to the clipboard as spoken.
        assert_eq!(
    ...
    fn removes_fillers_case_insensitively_and_with_punctuation() {
        assert_eq!(
            remove_disfluencies("So, um, we should go"),
            "So, we should go"
        );
    ...
    fn does_not_collapse_repeats_that_are_valid_english() {
        ...
        assert_eq!(
            remove_disfluencies("he had had enough"),
            "he had had enough"
        );
        assert_eq!(
            remove_disfluencies("I know that that is true"),
    ...
    fn restores_the_capital_when_a_leading_filler_goes() {
        assert_eq!(
            remove_disfluencies("Um so we start here"),
            "So we start here"
        );
    }
